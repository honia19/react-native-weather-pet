...
const CarbonPollutionView = inject(
  'locationStore',
  'pollutionAirStore'
)(
  observer(({ locationStore, pollutionAirStore }) => {
    ...
    const setPullutionInfo = (dataPollution: Object): void => {
      ...
      if (errorMsg) {
        ...
        pollutionAirStore.setCO2PollutionData(dataPollution.data);
      }
    };
    ...
    const getAitPollutionData = async (): Promise<void> => {
      try {
        const response = await fetch(
          `http://api.openweathermap.org/pollution/v1/co/0.0,10.0/2016-03-01Z.json?appid=${API_KEY}`
        );
...
export default CarbonPollutionView;
